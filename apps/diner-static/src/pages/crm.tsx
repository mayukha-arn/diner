...
export default function CRMPage() {
  ...
  const selected: Customer | null = CUSTOMERS.find((c) => c.id === selectedId) ?? null;
        ...
        <div className="flex gap-6">
          <div className="flex-1 bg-white rounded-2xl border-2 border-foreground overflow-hidden shadow-[0_8px_24px_rgba(26,16,8,0.08)]">
            <table className="w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={5} className="px-5 py-12 text-center font-sans text-muted-foreground">No customers found.</td></tr>
                ) : filtered.map((cust) => (
                  <tr
                    key={cust.id}
                  ...
                  >
                    ...
                    <td className="px-5 py-3 font-sans text-xs text-muted-foreground text-right">{new Date(cust.lastOrderDate).toLocaleDateString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <div className="p-5 space-y-5">
                <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-full bg-primary/10 border-2 border-primary/30 flex items-center justify-center">
                    <span className="font-display text-2xl font-bold text-primary">{selected.name.charAt(0)}</span>
                  </div>
                  <div>
                    <p className="font-display text-xl font-bold text-foreground">{selected.name}</p>
                    <p className="font-sans text-xs text-muted-foreground">{selected.email}</p>
                    {selected.phone && <p className="font-sans text-xs text-muted-foreground">{selected.phone}</p>}
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-background/50 rounded-xl p-3 border border-foreground/10">
                    ...
                    <p className="font-display text-xl font-bold text-foreground">{selected.orderCount}</p>
                  </div>
                  <div className="bg-background/50 rounded-xl p-3 border border-foreground/10">
                    <p className="font-sans text-xs text-muted-foreground mb-1">Total Spend</p>
                    <p className="font-display text-xl font-bold text-primary">${selected.totalSpend.toFixed(2)}</p>
                  </div>
                </div>
                <div>
                  ...
                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {selected.orders.map((order) => (
                      <div key={order.id} className="flex items-center justify-between bg-background/50 rounded-xl px-3 py-2 border border-foreground/10">
                        <div>
