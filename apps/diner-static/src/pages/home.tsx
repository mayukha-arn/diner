...
import { DASHBOARD, ORDERS } from "@/data";
...
export default function HomePage() {
  const kpis = [
    { label: "Total Orders", value: DASHBOARD.totalOrders, icon: ShoppingBag, color: "bg-primary", textColor: "text-white" },
    { label: "Revenue", value: `$${DASHBOARD.totalRevenue.toFixed(2)}`, icon: TrendingUp, color: "bg-[#F5C842]", textColor: "text-foreground" },
    { label: "Pending Orders", value: DASHBOARD.pendingOrders, icon: Clock, color: "bg-foreground", textColor: "text-background" },
    { label: "Popular Item", value: DASHBOARD.popularItem, icon: Star, color: "bg-white", textColor: "text-foreground" },
  ];
      ...
      <div className="p-8 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
          {kpis.map(({ label, value, icon: Icon, color, textColor }) => (
            <div key={label} className={`${color} ${textColor} rounded-2xl p-5 border-2 border-foreground shadow-[0_8px_24px_rgba(26,16,8,0.08)] flex flex-col gap-3`}>
        ...
        <div className="mb-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {DASHBOARD.topItems.map((item, i) => (
              <div key={item.id} className="bg-white rounded-2xl border-2 border-foreground p-5 flex flex-col items-center gap-3 shadow-[0_8px_24px_rgba(26,16,8,0.08)] hover:shadow-[0_12px_32px_rgba(26,16,8,0.14)] transition-shadow duration-200">
                <div className="[animation:float_3s_ease-in-out_infinite]" style={{ animationDelay: `${i * 0.4}s` }}>
                  {FOOD_SVG_MAP[item.name] ?? <SmashBurger />}
                </div>
            ...
            <table className="w-full">
              ...
              <tbody>
                {ORDERS.slice(0, 8).map((order) => (
                  <tr key={order.id} className="border-b border-foreground/5 hover:bg-background/50 transition-colors duration-100">
